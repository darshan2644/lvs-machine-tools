...
const CheckoutPage = () => {
  ...
  const processCOD = async () => {
    ...
    const orderSaved = saveOrder(orderData);
    if (orderSaved) {
  ...
  const processCard = async () => {
    ...
    if (orderSaved) {
      // Clear cart after successful order
      clearCart();
      alert(`Payment successful!\nOrder ID: ${orderId}\nTracking Number: ${orderData.trackingNumber}\nAmount: ${formatCurrency(calculations.grandTotal)}`);
      navigate('/order-confirmation', { state: { orderId, orderData } });
    } else {
      throw new Error('Failed to save order');
    }
    ...
    const options = {
      ...
      handler: async (response) => {
        try {
          ...
          const orderSaved = saveOrder(finalOrderData);
          if (orderSaved) {
            // Clear cart after successful order
            clearCart();
            alert(`Payment successful!\nOrder ID: ${orderId}\nTracking Number: ${orderData.trackingNumber}\nPayment ID: ${response.razorpay_payment_id}`);
            navigate('/order-confirmation', { state: { orderId, orderData: finalOrderData, paymentId: response.razorpay_payment_id } });
          } else {
